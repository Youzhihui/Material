...
<html>
  <head>
    ...
    <script type="text/javascript">
    	...
    	function change(state)
    	{
        	...
        	{
            	window.location.href="${pageContext.request.contextPath }/input/selectByName.do";
            }
        	else if(state == 4)
        	{
            	window.location.href="${pageContext.request.contextPath }/output/selectByName.do";
            }
        	else if(state == 2)
        	{
        		window.location.href="${pageContext.request.contextPath }/forward.do";
            }
        }
        ...
        function fun1(flag,id)
        {
        	if(flag == 1)
        	{
        		window.location.href = "selectinputById.do?id="+id+"&flag="+flag;
        	}
        	else if(flag == 2)
        	{
        		window.location.href = "selectoutputById.do?id="+id+"&flag="+flag;
            }
            else
            {
            	window.location.href = "selectProById.do?id="+id+"&flag="+flag;
            }
        }
       	//通过按钮 设置表单提交的action
        function chaxun2(){
        ...
        function changeTypes(bid){
    		...
    		$.getJSON("${pageContext.request.contextPath }/smalltype/selectByBid.do?bid="+bid,
    		function(data){
    			//清空下拉列表
    			$("#smalltype").empty();
    			//alert(data);
  ...
  <body>
  	<center>
	  	<div>
	   		<h1>物料库存管理系统${admin.name}</h1>
	   		<h2><font color="blue">
	   			<c:if test="${admin.state==-1}">
		   			<a href="${pageContext.request.contextPath }/selectadminAll.do">管理员管理</a>
	   			</c:if>
	   		</font></h2>
	   		 <table width="900" cellspacing="0" border="1">
	   			...
	   			<tr valign="middle">
		   				...
		   				<td>${p.wvalue}</td>
		   				<td>
		   				<fmt:formatDate pattern="yyyy-MM-dd hh:mm:ss" value="${p.createtime}" /></td>
		   				<td>
